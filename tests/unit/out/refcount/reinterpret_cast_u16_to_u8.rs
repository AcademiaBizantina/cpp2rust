...
fn main_0() -> i32 {
    let arr: Value<Box<[u16]>> = Rc::new(RefCell::new(Box::new([513_u16, 1027_u16, 1541_u16])));
    let bytes: Value<Ptr<u8>> = Rc::new(RefCell::new(
        (arr.as_pointer() as Ptr<u16>).reinterpret_cast::<u8>(),
    ));
    ...
    (if (((*arr.borrow())[(0) as usize] as i32) == 65281) {
    } else {
        std::panic::panic_any(Ptr::from_string_literal("arr[0] == 0xFF01").to_rust_string())
    });
    return 0;
}
